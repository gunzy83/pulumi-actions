...
describe('summary.ts', () => {
  ...
  it('should add only heading with empty code block to summary', async () => {
    const message = '';
    const expected = `<h3>Pulumi ${projectName}/${defaultOptions.stackName} has changes.</h3>\n<details><summary>Pulumi report</summary><pre lang="diff">\n\n</pre></details>\n`;

    await handleSummaryMessage(defaultOptions, projectName, message, true);
  ...
  it('should trim the output when the output is larger than 1 MiB', async () => {
    const message =
      'this is at the begining and should not be in the output' +
      'a'.repeat(1_048_576) +
  ...
  it('should trim the output from front when the output is larger than 1 MiB and config is set', async () => {
    ...
    await handleSummaryMessage(options, projectName, message, true);
    const summary = await getSummary();
